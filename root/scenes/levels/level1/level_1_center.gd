...
func _start_foam_sway(child: Sprite2D, i: int, delay: float = 0.0) -> void:
	...
	if delay > 0:
		sway_tween.tween_interval(delay)

	sway_tween.tween_property(child, "position:x", rest_x + sway_amp * sway_dir, sway_dur) \
		.set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
	sway_tween.tween_property(child, "position:x", rest_x - sway_amp * sway_dir, sway_dur) \
		.set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
...
func _start_duck_idle(duck: Sprite2D, rest_pos: Vector2, rest_rot: float, start_delay: float = 0.0) -> void:
	...
	if start_delay > 0: bob_tween.tween_interval(start_delay)
	...
	if start_delay > 0: rot_tween.tween_interval(start_delay)
