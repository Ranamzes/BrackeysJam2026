...
var _gdlint_command: String
var _gdlint_base_args: Array[String] = []
...
func _enter_tree() -> void:
	...
	setup_gdlint_command()
	get_gdlint_version()
	prints("Loading GDLint Plugin success")


func setup_gdlint_command() -> void:
	# Check for uv first as requested
	var output := []
	var err = OS.execute("uv", ["run", "gdlint", "--version"], output)
	if err == OK:
		_gdlint_command = "uv"
		_gdlint_base_args = ["run", "gdlint"]
		return

	# Fallback to old behavior
	_gdlint_command = get_gdlint_path()
	_gdlint_base_args = []
...
func get_gdlint_version() -> void:
	var output := []
	var args := _gdlint_base_args + ["--version"]
	var err = OS.execute(_gdlint_command, args, output)
	if err != OK:
		_is_gdlint_installed = false
		_dock_ui.version.text = "gdlint not found"
		return

	_is_gdlint_installed = true if output.size() > 0 and not output[0].is_empty() else false
	if _is_gdlint_installed:
		var version_text = output[0].strip_edges()
		if _gdlint_command == "uv":
			version_text += " (uv)"
		_dock_ui.version.text = "Using %s" % version_text
	else:
		_dock_ui.version.text = "gdlint not found!"
...
func on_resource_saved(resource: Resource) -> void:
	...
	var args := _gdlint_base_args + [filepath]
	var exit_code = OS.execute(_gdlint_command, args, gdlint_output, true)
	if not exit_code == -1:
